...
export class DashboardComponent implements OnInit {
  ...
  getListStock() {
    this.spinner.show();
    this.stocksService.list().subscribe(
      (data) => {
        if (data.status === 200) {
  ...
  getCoinListLiveChanges() {
    this.spinner.show();
    this.globalService.setLoadingLabel(
      "Data is fetching with live... Please Wait."
    );
    this.cryptoService.getCryptoSymbolLiveDetails().subscribe(
      (data) => {
        if (data.status === 200) {
